...
int bind_gpu_cube_texture(int textureId)
{
	glBindTexture(GL_TEXTURE_CUBE_MAP, textureId);
	return 0;
}
...
void bind_bound_point_vbobuf(unsigned int VBOid, unsigned int EBOid)
{
	glBindBuffer(GL_ARRAY_BUFFER, VBOid);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBOid);
}

void gen_bound_point_buf(unsigned int &VBOid, unsigned int &EBOid, vector<Vertex>& vertices, vector<unsigned short>& indices)
{
...
void skybox_vbo(unsigned int &VBOid, float *skyboxVertices1)
{
...
void draw_bound(int line_poin_size)
{
	//glDrawArrays(GL_LINES, 0, line_poin_size);
	glDrawElements(GL_LINES, line_poin_size, GL_UNSIGNED_SHORT, 0);
	//glDrawArrays(GL_LINE_STRIP, 0, 8);
}

void draw_array(int size)
{
